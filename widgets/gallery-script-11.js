...
.lightbox-arrow {
      ...
      thumbnailContainer.querySelectorAll('.thumbnail').forEach(thumb => {
        thumb.addEventListener('click', () => {
          if (!isAnimating) {
            ...
            showLightbox(parseInt(thumb.getAttribute('data-index')), parseInt(thumb.getAttribute('data-index')) > currentIndex ? 'right' : 'left');
          }
        });
      });
    }
    ...
    function showLightbox(index, direction = 'none') {
      ...
      const oldImg = lightbox.querySelector('.lightbox-img');
      const incoming = document.createElement('img');
      incoming.src = newSrc;
      incoming.alt = newAlt;
      incoming.className = 'lightbox-img';
      incoming.style.transform = direction === 'left' ? 'translateX(-100%)' : direction === 'right' ? 'translateX(100%)' : 'translateX(0)';
      ...
      if (oldImg) {
        oldImg.style.transform = direction === 'left' ? 'translateX(100%)' : direction === 'right' ? 'translateX(-100%)' : 'translateX(0)';
      }

      incoming.addEventListener('transitionend', function handler() {
        incoming.removeEventListener('transitionend', handler);
        if (oldImg && oldImg !== incoming) {
